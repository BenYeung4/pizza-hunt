...
//setting up the get function to retireve individual pizza and it's discussion
function getPizza() {
  //get id of pizza
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    .then((response) => {
      ...
      if (!response.ok) {
        //this is in case a user's pizza can't be found in the database.  if there is no id or pizza cannot be found, an error will be thrown
        throw new Error({ message: "Something went wrong!" });
      }
      return response.json();
    })
    .then(printPizza)
    //this catch is so that any error takes the user back to th homepage, using the window.history.back() method
    .catch((err) => {
      console.log(err);
...
function handleNewCommentSubmit(event) {
  ...
  //this lets to write new comment
...
function handleNewReplySubmit(event) {
  ...
  //this handle PUT requests for new replies, so it can be entered in the front end
...
//added for the function to work
getPizza();
